...
```bash
( mkdir -p ~/local && cd "$_" && if [[ -d dotfiles ]]; then cd dotfiles && git pull --rebase --prune; else git clone -c core.autocrlf=input --filter=tree:0 --single-branch --branch main --no-tags https://github.com/yukinobu/dotfiles.git; fi )
